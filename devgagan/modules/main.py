...
async def initialize_userbot(user_id): # this ensure the single startup .. even if logged in or not
    ...
    if data and data.get("session"):
        try:
            ...
            await app.send_message(user_id, "Login Expired.Login again using /login.")
            return None
    else:
        if DEFAULT_SESSION:
            return userrbot
...
async def batch_link(_, message):
    ...
    join_button = InlineKeyboardButton("Join Channel", url="https://t.me/ihtars_updates")
    keyboard = InlineKeyboardMarkup([[join_button]])
    pin_msg = await app.send_message(
        user_id,
        f"Batch process started ⚡\nProcessing: 0/{cl}\n\n**Powered by Team Ihtar's Community.**",
            ...
            if user_id in users_loop and users_loop[user_id]:
                ...
                if 't.me/' in link and not any(x in link for x in ['t.me/b/', 't.me/c/', 'tg://openmessage']):
                    ...
                    await pin_msg.edit_text(
                        f"Batch process started ⚡\nProcessing: {i - cs + 1}/{cl}\n\n**__Powered by Ihtar's Community.__**",
                        reply_markup=keyboard
                    )
